...
def _inplace_mgs_qr(s):
    """
    修正格拉姆-施密特正交化 (Modified Gram-Schmidt).
    原地修改矩阵 s, 不产生额外的 N 维度大矩阵。
    """
    n, k = s.shape
    ...
    for i in range(k):
        ...
        # 3. 减去后续列在第 i 列上的投影 (逐列操作，显存友好)
        if i + 1 < k:
            # 这里的 s[:, i:i+1].conjT @ s[:, i+1:] 产生的临时矩阵很小 (1 x (k-i-1))
            projections = s[:, i].T.conj() @ s[:, i+1:]
            r[i, i+1:] = projections
            # 原地更新后续列: s_j = s_j - <s_i, s_j> * s_i
